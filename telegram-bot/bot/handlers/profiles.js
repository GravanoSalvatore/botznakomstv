...
const { AGE_RANGES, FALLBACK_COUNTRIES } = require('./config/constants');
const { 
...
const replaceSitesInAbout = (aboutText) => {
  ...
  const cleanedAbout = aboutText.replace(
    siteRegex,
    "http://t.me/magicboss_bot/magic"
  );
...
class AsyncFilterManager {
  ...
  async filterProfilesAsync(profiles, filters, isDemo = false) {
    return this.filterQueue.add(async () => {
      ...
      // 5. ПЕРИОДИЧЕСКИЙ ОТЧЕТ СТАТИСТИКИ
  ...
  applyFiltersWithLMDBIndex(filters, isDemo = false) {
    ...
    const normalizedFilterCity = filters.city
      ? cacheManager.normalizeCityName(filters.city)
      : null;
  ...
  async applyFiltersParallel(profiles, filters) {
    return new Promise((resolve) => {
      ...
      // Разбиваем на 4 части (оптимально для Node.js)
      const CHUNKS = 4;
      ...
      for (let chunkIndex = 0; chunkIndex < CHUNKS; chunkIndex++) {
        ...
        setImmediate(() => {
          ...
          // Фильтрация внутри чанка
          for (let i = 0; i < chunk.length; i++) {
            const profile = chunk[i];

            // Быстрая проверка страны
  ...
  const getProfilesPage = async (page = 0, searchCountry = null, ageRange = null, searchCity = null, isDemo = false) => {
    try {
      ...
      const normalizedSearchCity = searchCity
        ...
        : null;
      const normalizedSearchCountry = searchCountry
      ...
      return messageQueue.add(async () => {
        ...
        try {
          ...
          const totalPages = Math.ceil(
            uniqueCountries.length / countriesPerPage
          );
    ...
    sendCitiesKeyboard: async function (ctx, country, isDemo = false) {
      return messageQueue.add(async () => {
        ...
        try {
          ...
          if (!cities || cities.length === 0) {
            const msg = await ctx.reply(
              `❌ Для страны "${country}" нет доступных городов`
            );
            self.track(chatId, msg.message_id);
            return;
          }
          ...
          const createCitiesKeyboard = (page) => {
            ...
            // 🔥 ДОБАВЛЯЕМ НОВУЮ КНОПКУ "🔙 Назад к странам"
            keyboard.push([
              {
            ...
            keyboard.push([
              {
                text: "📝 СОЗДАТЬ АНКЕТУ",
                web_app: {
                  url: "https://bot-vai-web-app.web.app/?tab=catalog",
                },
              },
            ]);
          ...
          const msg = await ctx.reply(msgText, {
            reply_markup: {
              inline_keyboard: createCitiesKeyboard(currentPage),
            },
          });
  ...
  bot.command("fix_city", async (ctx) => {
    try {
      ...
      if (!testCity || !testCountry) {
        return ctx.reply("Например: /fix_city Дубай ОАЭ");
      }
      ...
      // 4. Проверяем есть ли город в списке
      ...
      response += `📋 **РЕЗУЛЬТАТЫ:**\n`;
      response += `• Город в списке: ${cityExists ? "✅" : "❌"}\n`;
      ...
      if (!cityExists) {
        ...
        // Ищем похожие
        const similarCities = Array.from(citiesInCountry).filter(
          (city) =>
        ...
        if (variants.length > 0) {
          response += `Варианты написания в БД:\n`;
   ...
   bot.command("check_country_normalization", async (ctx) => {
    try {
      ...
      allProfiles.forEach((p) => {
        ...
        if (
          country &&
          (country.includes("ОАЭ") ||
            country.includes("UAE") ||
            country.includes("Эмират"))
        ) {
      ...
      if (uaeVariants.size > 0) {
        ...
        uaeProfiles.slice(0, 5).forEach((p, i) => {
          response += `${i + 1}. ${p.name}\n`;
          response += `   Страна: "${p.country}"\n`;
          response += `   Нормализовано: ${p.normalized}\n`;
          response += `   Город: "${p.city}"\n\n`;
        });
        ...
        response += `• Нормализованный: "${normalizedFilter}"\n`;

        const matchingProfiles = uaeProfiles.filter(
          (p) =>
            cacheManager.normalizeCountryName(p.country) === normalizedFilter
        );

        response += `• Совпадает с ${matchingProfiles.length} профилями\n`;
      }

      await ctx.reply(response);
    } catch (error) {
      console.error("Ошибка check_country_normalization:", error);
 ...
 bot.action("filter_by_age", async (ctx) => {
    ...
    await messageQueue.add(async () => {
      try {
        ...
        keyboard.push([
          {
            text: "❌ Сбросить фильтр возраста",
            callback_data: "age_range_reset",
          },
        ]);
  ...
  bot.action(/^age_range_(.+)$/, async (ctx) => {
    ...
    await messageQueue.add(async () => {
      try {
        ...
        if (!profiles.length) {
          const msg = await ctx.reply(
            "Анкет по выбранным критериям не найдено."
          );
          messageManager.track(ctx.chat.id, msg.message_id);
          return;
        }
        ...
        if (isDemo) {
          filtersText += "\n👀 ДЕМО-РЕЖИМ: показано максимум  по 3 анкеты на город";
        }
  ...
  bot.action("check_subscription_now", async (ctx) => {
    await messageQueue.add(async () => {
      try {
        ...
        console.log(
          `🔍 [SUBSCRIPTION CHECK] Пользователь ${userId} проверяет подписку`
        );
...
❌ <b>Но нет подписки на канал <a href="https://t.me/+H6Eovikei9xiZWU0"><b>MagicClubPrivate</b></a></b>

Что нужно сделать:
1. Подпишитесь на <a href="https://t.me/+H6Eovikei9xiZWU0"><b>MagicClubPrivate</b></a>
...
bot.action(/^page_(first|prev|next|last|\d+)_(\d+)$/, async (ctx) => {
  ...
  await messageQueue.add(async () => {
    try {
      ...
      const MAX_SAFE_PAGE = 10000; // 👈 Вместо 9243 ставим безопасный лимит
...
<code>Нажмите "🌍 ВСЕ СТРАНЫ" чтобы начать</code>
          ...
          if (!hasChannelSubscription) missing.push("подписки на канал ");
...
${
  !hasSubscription
    ? "1. 💎 Купите подписку через 'ПОЛУЧИТЬ ПОЛНЫЙ ДОСТУП'\n"
    : ""
}
${!hasChannelSubscription ? "2. 📢 Подпишитесь на канал \n" : ""}
