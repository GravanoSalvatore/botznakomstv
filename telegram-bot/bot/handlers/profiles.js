...
const lazyLoadFullCache = async (userId = 'system', force = false) => {
    ...
    //     // 🔥 🔥 🔥 ЕСЛИ forceRefresh = true, ОЧИЩАЕМ ТОЛЬКО КЭШ КАНАЛА
    //     if (forceRefresh) {
    //         console.log(`🔄 [FORCE REFRESH] Очищаем кэш канала для ${userId}`);
    //         channelSubscriptionCache.del(`channel:${userId}`);
            
    //         // Запоминаем что пользователь запросил проверку
    //         if (!ctx.session) ctx.session = {};
    //         ctx.session.forceChannelCheck = true;
    //         ctx.session.recentChannelCheck = Date.now();
    //     }
        
    //     // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ БАГА: ГАРАНТИРОВАННЫЙ ДЕМО-КЭШ ДЛЯ НОВЫХ ПОЛЬЗОВАТЕЛЕЙ
    //     if (!ctx.session) ctx.session = {};
        
    //     // Проверяем текущий статус в кэше пользователя (7 дней)
    //     const currentCacheStatus = cacheManager.getUserCacheStatus(userId);
    //     console.log(`🔍 [CACHE STATUS] Пользователь ${userId}: текущий статус = ${currentCacheStatus || 'не установлен'}`);
        
    //     // 🔥 🔥 🔥 КРИТИЧЕСКИЙ ФИКС: ЕСЛИ ДЕМО-КЭШ НЕ ЗАГРУЖЕН - СРОЧНО ЗАГРУЖАЕМ!
    //     if (!cacheManager.isDemoCacheLoaded()) {
    //         console.error(`💥 [CRITICAL] Демо-кэш не загружен! Срочно загружаем...`);
    //         try {
    //             await loadProfileCache(db, false);
    //             console.log(`✅ [CRITICAL] Демо-кэш срочно загружен`);
    //         } catch (error) {
    //             console.error(`❌ [CRITICAL] Не удалось загрузить демо-кэш:`, error);
    //             // В случае ошибки все равно возвращаем демо
    //         }
    //     }
        
    //     // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ БАГА: ПРОВЕРЯЕМ СЕССИОННЫЙ КЭШ
    //     if (ctx.session.cacheSession) {
    //         const sessionCache = ctx.session.cacheSession;
    //         const sessionAge = Date.now() - sessionCache.timestamp;
            
    //         // Сессионный кэш действителен 15 минут (или до принудительного обновления)
    //         if (!forceRefresh && sessionAge < 15 * 60 * 1000) {
    //             console.log(`✅ [SESSION CACHE] Используем сессию для ${userId}: ${sessionCache.type} (${Math.round(sessionAge/1000)} сек)`);
                
    //             // 🔥 🔥 🔥 ВАЖНО: Если в сессии fullAccess = false, но cacheType = 'full' - исправляем
    //             if (sessionCache.type === 'full' && sessionCache.fullAccess === false) {
    //                 console.log(`🔄 [SESSION FIX] Исправляем баг сессии: fullAccess=false но cacheType=full`);
    //                 sessionCache.type = 'demo';
    //                 return 'demo';
    //             }
    ...
    //     try {
    //         console.log(`🔍 [ACCESS CHECK] Полная проверка доступа для ${userId} (force: ${forceRefresh})`);
            
    //         // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ БАГА: ПРАВИЛЬНЫЙ ПОРЯДОК ПРОВЕРОК
    //         let hasSubscription = false;
    //         let hasChannelSubscription = false;
    //         let cacheType = 'demo'; // ПО УМОЛЧАНИЮ - ДЕМО!
            
    //         // 1. СНАЧАЛА ПРОВЕРЯЕМ КАНАЛ (Telegram API)
    //         console.log(`📢 [CHECK 1] Проверяем канал для ${userId}`);
    //         hasChannelSubscription = await checkChannelSubscription(ctx);
    //         console.log(`📢 [CHECK 1] Результат: канал = ${hasChannelSubscription}`);
            
    //         // 🔥 🔥 🔥 КРИТИЧЕСКИЙ ФИКС: ЕСЛИ НЕТ КАНАЛА - СРАЗУ ДЕМО!
    //         if (!hasChannelSubscription) {
    //             console.log(`❌ [NO CHANNEL] Пользователь ${userId} не подписан на канал -> ДЕМО-РЕЖИМ`);
    //             cacheType = 'demo';
                
    //             // 🔥 🔥 🔥 ВАЖНО: НО ПРОВЕРЯЕМ ПОДПИСКУ ЧТОБЫ ПОКАЗАТЬ ПРАВИЛЬНОЕ СООБЩЕНИЕ
    //             hasSubscription = await checkSubscription(userId);
                
    //             // Сохраняем в сессионный кэш доступа
    //             ctx.session.fullAccessCache = {
    //                 value: false, // 🔥 ПОЛНЫЙ ДОСТУП = false!
    //                 timestamp: Date.now(),
    ...
    //             console.log(`✅ [ACCESS] ${userId}: подписка=${hasSubscription}, канал=false, доступ=ДЕМО`);
    //             return 'demo';
    //         }
            
    //         // 2. ЕСЛИ КАНАЛ ЕСТЬ - ПРОВЕРЯЕМ ПОДПИСКУ
    //         console.log(`💎 [CHECK 2] Проверяем подписку для ${userId}`);
    //         hasSubscription = await checkSubscription(userId);
    //         console.log(`💎 [CHECK 2] Результат: подписка = ${hasSubscription}`);
            
    //         // 3. ОПРЕДЕЛЯЕМ ТИП КЭША
    //         cacheType = (hasSubscription && hasChannelSubscription) ? 'full' : 'demo';
            
    //         console.log(`📊 [FINAL ACCESS] ${userId}: подписка=${hasSubscription}, канал=${hasChannelSubscription}, доступ=${cacheType}`);
            
    //         // 🔥 🔥 🔥 ВАЖНО: ЛОГИКА ПОЛНОГО ДОСТУПА
    //         if (cacheType === 'full') {
    //             console.log(`🎉 [FULL ACCESS] Пользователь ${userId} получил полный доступ`);
                
    //             // Проверяем, загружен ли полный кэш
    //             if (!cacheManager.isFullCacheLoaded()) {
    //                 console.log(`🚀 [FULL CACHE NEEDED] Загружаем полный кэш для ${userId}`);
                    
    //                 // Показываем сообщение о загрузке
    //                 if (ctx.callbackQuery) {
    //                     try {
    //                         await ctx.answerCbQuery("🔄 Загружаем полный доступ...");
    //                     } catch (e) {}
    //                 }
    ...
    //                     console.log(`✅ [FULL CACHE] Полный кэш загружен для ${userId}`);
    //                 } catch (error) {
    //                     console.error(`❌ [FULL CACHE ERROR] Не удалось загрузить:`, error.message);
    //                     // Если не удалось загрузить - остаемся в демо
    //                     cacheType = 'demo';
    //                 }
    //             }
    //         } else {
    //             console.log(`👀 [DEMO ACCESS] Пользователь ${userId} в демо-режиме`);
                
    //             // 🔥 🔥 🔥 ВАЖНО: Если канал есть, но подписки нет
    //             if (hasChannelSubscription && !hasSubscription) {
    //                 console.log(`⚠️ [PARTIAL] У ${userId} есть канал, но нет подписки`);
    //             }
    //         }
            
    //         // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ БАГА: ОБНОВЛЯЕМ ВСЕ КЭШИ
    //         cacheManager.setUserCacheStatus(userId, cacheType);
            
    //         // Сохраняем в сессионный кэш доступа
    //         ctx.session.fullAccessCache = {
    //             value: (cacheType === 'full'),
    //             timestamp: Date.now(),
    //             subscription: hasSubscription,
    ...
    //             type: cacheType,
    //             timestamp: Date.now(),
    //             fullAccess: (cacheType === 'full'),
    //             subscription: hasSubscription,
    //             channel: hasChannelSubscription
    //         };
            
    //         console.log(`✅ [CACHE UPDATED] ${userId}: тип кэша = ${cacheType}`);
    //         return cacheType;
            
    //     } catch (error) {
    //         console.error(`❌ [CACHE CHECK ERROR] Ошибка для ${userId}:`, error);
            
    //         // 🔥 🔥 🔥 ВАЖНО: ПРИ ЛЮБОЙ ОШИБКЕ - ВОЗВРАЩАЕМ ДЕМО!
    //         cacheManager.setUserCacheStatus(userId, 'demo');
    ...
    //         console.log(`⚠️ [FAILSAFE] ${userId} переведен в демо-режим из-за ошибки`);
    //         return 'demo';
    //     }
    // };
// ===================== ОПТИМИЗИРОВАННАЯ ФУНКЦИЯ ПРОВЕРКИ И ОБНОВЛЕНИЯ КЭША =====================
...
//     try {
//         // 🔥 🔥 🔥 ЭТАП 1: БЫСТРАЯ ИНИЦИАЛИЗАЦИЯ (БЕЗ ПРОВЕРОК)
//         await ctx.answerCbQuery("🌍 Загружаем страны...");
        
//         // Очищаем предыдущие сообщения
//         await messageManager.clear(ctx);
        
//         // 🔥 🔥 🔥 ЭТАП 2: ОПРЕДЕЛЯЕМ РЕЖИМ РАБОТЫ
//         let cacheType = cacheManager.getUserCacheStatus(userId);
//         let isDemo = true; // 🔥 ПО УМОЛЧАНИЮ ДЕМО!
        
//         console.log(`📊 [CACHE INIT] Пользователь ${userId}: текущий статус = ${cacheType || 'нет'}`);
        
//         // Если у пользователя еще нет статуса - быстрая инициализация
//         if (!cacheType) {
//             console.log(`👤 [NEW USER] Быстрая инициализация для ${userId}`);
            
//             // 🔥 СУПЕР-БЫСТРАЯ ЛОГИКА: СНАЧАЛА ПРОВЕРЯЕМ СЕССИЮ
//             if (ctx.session?.fullAccessCache) {
//                 const accessCache = ctx.session.fullAccessCache;
//                 const accessAge = Date.now() - accessCache.timestamp;
                
//                 if (accessAge < 15 * 60 * 1000) {
//                     // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ: ЕСЛИ НЕТ КАНАЛА - ДЕМО!
//                     cacheType = (accessCache.channel && accessCache.subscription) ? 'full' : 'demo';
//                     isDemo = cacheType === 'demo';
//                     console.log(`✅ [SESSION HIT] Статус из сессии: ${cacheType}, канал=${accessCache.channel}, подписка=${accessCache.subscription}`);
//                 }
//             }
            
//             // Если нет в сессии - по умолчанию ДЕМО (без проверок!)
//             if (!cacheType) {
//                 cacheType = 'demo';
//                 isDemo = true;
//                 console.log(`✅ [DEFAULT] Установлен демо по умолчанию (новый пользователь)`);
//             }
...
//         // 🔥 🔥 🔥 ЭТАП 3: ПРОВЕРЯЕМ НУЖНО ЛИ ПЕРЕПРОВЕРИТЬ КАНАЛ
//         let shouldForceCheckChannel = false;
        
//         // 🔥 КРИТИЧЕСКИЙ ФИКС: ЕСЛИ СТАТУС ПОЛЬЗОВАТЕЛЯ 'full', НО В СЕССИИ НЕТ КАНАЛА
//         if (cacheType === 'full' && ctx.session?.fullAccessCache && !ctx.session.fullAccessCache.channel) {
//             console.log(`⚠️ [CACHE INCONSISTENCY] Статус full, но в сессии нет канала! Исправляем...`);
//             shouldForceCheckChannel = true;
//             cacheType = 'demo'; // 🔥 ВРЕМЕННО МЕНЯЕМ НА ДЕМО
//             isDemo = true;
//         }
        
//         // 🔥 ПРОВЕРКА 1: Пользователь недавно нажал "проверить подписку"
//         if (isDemo && ctx.session?.recentChannelCheck) {
//             const timeSinceCheck = Date.now() - ctx.session.recentChannelCheck;
//             if (timeSinceCheck < 5 * 60 * 1000) { // В последние 5 минут
//                 shouldForceCheckChannel = true;
//                 console.log(`🔄 [FORCE CHECK] Пользователь недавно проверял подписку`);
//             }
//         }
        
//         // 🔥 ПРОВЕРКА 2: В сессии есть данные о подписке, но нет о канале
//         if (ctx.session?.fullAccessCache?.subscription && !ctx.session?.fullAccessCache?.channel) {
//             shouldForceCheckChannel = true;
//             console.log(`🔄 [FORCE CHECK] У пользователя есть подписка, но статус канала неясен`);
//         }
        
//         // 🔥 ПРОВЕРКА 3: Пользователь явно запросил проверку
//         if (ctx.session?.forceChannelCheck) {
//             shouldForceCheckChannel = true;
//             delete ctx.session.forceChannelCheck;
//             console.log(`🔄 [FORCE CHECK] Принудительная проверка по запросу`);
//         }
        
//         // 🔥 УДАЛЯЕМ ПЕРВЫЙ ПРЕЛОАДЕР ПОСЛЕ БЫСТРОЙ ПРОВЕРКИ
//         if (accessPreloader) {
//             try {
//                 await ctx.telegram.deleteMessage(ctx.chat.id, accessPreloader.message_id);
//             } catch (e) {
...
//         // 🔥 🔥 🔥 ЭТАП 4: ЕСЛИ СЕССИЯ АКТУАЛЬНА - ИСПОЛЬЗУЕМ ЕЕ
//         if (ctx.session?.cacheSession && !shouldForceCheckChannel) {
//             const sessionCache = ctx.session.cacheSession;
//             const sessionAge = Date.now() - sessionCache.timestamp;
...
//                 // 🔥 🔥 🔥 ИСПРАВЛЕНИЕ: Проверяем корректность сессии
//                 if (sessionCache.type === 'full' && (!sessionCache.channel || !sessionCache.subscription)) {
//                     console.log(`⚠️ [SESSION BUG] Сессия некорректна: full без канала/подписки`);
//                     shouldForceCheckChannel = true;
//                 } else {
//                     // 🔥 Показываем информационное сообщение в зависимости от режима
//                     if (sessionCache.type === 'demo') {
//                         if (sessionCache.subscription && !sessionCache.channel) {
//                             // 🔥 🔥 🔥 СПЕЦИАЛЬНЫЙ СЛУЧАЙ: ЕСТЬ ПОДПИСКА, НЕТ КАНАЛА
//                             await ctx.reply(`
// 🎯 <b>ВАШ СТАТУС</b>
...
//                                 }
//                             });
//                         } else if (!sessionCache.subscription && !sessionCache.channel) {
//                             // 🔥 НЕТ ПОДПИСКИ И НЕТ КАНАЛА
//                             await ctx.reply(`
// 👀 <b>ДЕМО-РЕЖИМ</b>
...
//                     if (sessionCache.type === 'full' && !cacheManager.isFullCacheLoaded()) {
//                         console.log(`🚀 [LAZY FULL CACHE] Загружаем полный кэш для ${userId}`);
                        
//                         const loadingMsg = await sendPreloader(ctx, 'default', "🔄 ЗАГРУЗКА ПОЛНОГО ДОСТУПА");
...
//                     // 🔥 ПОКАЗЫВАЕМ СТРАНЫ
//                     await messageManager.sendCountriesKeyboard(ctx, sessionCache.type === 'demo', 0);
//                     return;
//                 }
//             }
//         }
        
//         // 🔥 🔥 🔥 ЭТАП 5: СЕССИЯ УСТАРЕЛА ИЛИ НУЖНА ПРОВЕРКА
//         console.log(`🔍 [SESSION UPDATE] Обновляем статус для ${userId} (force: ${shouldForceCheckChannel})`);
        
//         // 🔥 ДОБАВЛЯЕМ ВТОРОЙ ПРЕЛОАДЕР ДЛЯ ПРОВЕРКИ ДОСТУПА
//         const statusPreloader = await sendPreloader(ctx, 'access', "🔄 ОБНОВЛЕНИЕ ДОСТУПА");
        
//         try {
//             // 🔥 Используем оптимизированную проверку С ПРИНУДИТЕЛЬНОЙ ПРОВЕРКОЙ КАНАЛА
//             const newCacheType = await ensureProperCache(ctx, shouldForceCheckChannel);
//             const newIsDemo = newCacheType === 'demo';
            
//             console.log(`✅ [STATUS UPDATED] ${userId}: ${newCacheType}, демо=${newIsDemo}`);
            
//             // 🔥 УДАЛЯЕМ ПРЕЛОАДЕР ПОСЛЕ ПРОВЕРКИ
//             if (statusPreloader) {
//                 await ctx.telegram.deleteMessage(ctx.chat.id, statusPreloader.message_id);
//             }
            
//             // 🔥 🔥 🔥 ЭТАП 6: ЕСЛИ ВСЁ ЕЩЁ ДЕМО - ПОКАЗЫВАЕМ ПРАВИЛЬНОЕ СООБЩЕНИЕ
//             if (newIsDemo) {
//                 const access = ctx.session?.fullAccessCache;
...
//                         }
//                     });
//                 } else if (access.subscription && !access.channel) {
//                     // 🔥 🔥 🔥 КРИТИЧЕСКИЙ СЛУЧАЙ: ЕСТЬ ПОДПИСКА, НЕТ КАНАЛА
//                     await ctx.reply(`
// 🎯 <b>ВАШ СТАТУС</b>
...
// <code>Ваша подписка активна, но без подписки на канал доступ ограничен</code>
//                     `, { 
//                         parse_mode: "HTML",
//                         reply_markup: {
//                             inline_keyboard: [
...
//                 // 🔥 ВСЕГДА ПОКАЗЫВАЕМ ДЕМО-КЭШ
//                 await messageManager.sendCountriesKeyboard(ctx, true, 0);
//                 return;
//             }
            
//             // 🔥 🔥 🔥 ЭТАП 7: ЕСЛИ ПОЛНЫЙ ДОСТУП, НО КЭШ НЕ ЗАГРУЖЕН
...
//             // 🔥 🔥 🔥 ЭТАП 8: ПОЛНЫЙ ДОСТУП УСПЕШНО ЗАГРУЖЕН
//             if (!newIsDemo) {
//                 await ctx.reply(`
// ✅ <b>ПОЛНЫЙ ДОСТУП АКТИВИРОВАН!</b>
...
//         // 🔥 УДАЛЯЕМ ПРЕЛОАДЕР ПРИ КРИТИЧЕСКОЙ ОШИБКЕ
//         if (accessPreloader) {
//             try {
//                 await ctx.telegram.deleteMessage(ctx.chat.id, accessPreloader.message_id);
//             } catch (e) {}
...
//             // 🔥 🔥 🔥 КРИТИЧЕСКАЯ ОШИБКА - ВСЕГДА ДЕМО!
//             await ctx.reply(`
// ❌ <b>КРИТИЧЕСКАЯ ОШИБКА</b>
